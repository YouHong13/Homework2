...
uint8_t RemoteIP[] = {192,168,50,116};
#define RemotePORT	7002
...
int main(void)
{
  ...
  while(1)
  {
      if(Socket != -1)
      {
          ...
          int16_t pDataXYZ[3];
          char accelData[50];  // Buffer for sending accelerometer data

          for (;;)
          {
              BSP_ACCELERO_AccGetXYZ(pDataXYZ);
              snprintf(accelData, sizeof(accelData), "%d,%d,%d\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
              TERMOUT("Accelero X: %d   Y:%d    Z:%d\n\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);

              // Send accelerometer data to server
              ret = WIFI_SendData(Socket, (uint8_t*)accelData, strlen(accelData), &Datalen, WIFI_WRITE_TIMEOUT);
              if (ret != WIFI_STATUS_OK)
              {
              ...
              HAL_Delay(100); // Small delay to avoid flooding the server
          }
      }
  }
